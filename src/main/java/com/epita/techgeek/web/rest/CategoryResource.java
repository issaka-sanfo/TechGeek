...
import com.epita.techgeek.service.CategoryService;
...
/**
 * REST controller for managing {@link com.epita.techgeek.domain.Category}.
 */
@RestController
@RequestMapping("/api")
public class CategoryResource {
    ...
    public ResponseEntity<List<CategoryDTO>> getAllCategories(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("module-is-null".equals(filter)) {
            log.debug("REST request to get all Categorys where module is null");
            return new ResponseEntity<>(categoryService.findAllWhereModuleIsNull(),
                    HttpStatus.OK);
        }
